...
{
    std::unordered_map<std::tuple<int, int>, std::tuple<int, int>> ranges = {
        {{0, 1}, {-5, 10}},
        {{0, 2}, {-4, 12}},
        {{0, 3}, {-3, 14}},
        {{0, 4}, {-1, 15}},
    ...
    for (int i = 0; i < fs->getSize(); i++)
    {
        ...
        if (f1->isNoteBeingPlayed())
        {
            for (int j = 0; j < i; i++)
            {
                ...
                {
                    std::tuple<int, int> key{f1->getID(), f2->getID()};
                    int stretch_val = stretch(f1, f2);
                    std::tuple<int, int> range = ranges[key];
                    if (stretch_val < std::get<0>(range))
                    {
                        cost += 10 * (std::get<0>(range) - stretch_val);
                    }
                    else if (stretch_val > std::get<1>(range))
                    {
                        cost += 10 * (std::get<1>(range) - stretch_val);
                    }
                }
            }
        }
